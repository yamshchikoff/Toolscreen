...
- **Вопрос → ответ.** Если пользователь задаёт вопрос — агент отвечает на него. Не начинает действовать, не запускает команды, не пишет код. Только ответ.
...
Перед реализацией любой функциональности пишется юнит-тест на нее. Перед. Перед - это не фигура речи. Перед - это строгое протокольное требование. Перед - означает, что сначала делается тест, и только после того как тест сделан, пишется функциональность. Именно в такой последовательности. Сначала тест, потом функциональность. В любом плане, который пишется через plan mode, пункт про тест должен находиться перед пунктом про функциональность и иметь меньший порядковый номер. Тест после написания запускается, и должен падать. Тест после реализации функциональности запускается и не должен падать. Это называется TDD.
...
- **Отладка через GDB — по протоколу в [`docs/gdb_debugging.md`](docs/gdb_debugging.md).** GDB аттачится только после захода в мир. Перед аттачем — попросить перезапустить игру.
- **Запрещено предлагать batch-режим gdb.** Если возникают трудности с gdbwrap — перечитать [`docs/gdb_debugging.md`](docs/gdb_debugging.md) и [`docs/gdbwrap_usage.md`](docs/gdbwrap_usage.md). Batch-mode не используется для отладки.
- **Только gdbwrap для отладки.** Запрещено предлагать логи, printf, или любые другие средства кроме gdb через gdbwrap. Если gdbwrap имеет идентифицируемые проблемы — предложить исправления в сам gdbwrap.
- **PID определяется заново перед каждым подключением.** Нельзя использовать «текущий» или «старый» PID. Если кажется что PID не изменился — спросить разрешение. Любое переиспользование PID — под серьёзным подозрением.
- **При отладке пользовательских нажатий нельзя предлагать вернуться в игру с активными брикпойнтами.** Брикпойнт заморозит игру, пользователь залипнет. Только dprintf/tracepoint (не останавливают), либо брикпойнт ставится ПОСЛЕ того как пользователь уже в игре и нажал клавиши.
- **Запрещено просить пользователя вернуться в игру, когда установлен хотя бы один брикпойнт.** Перед возвратом проверить `info break` — если есть любой `breakpoint` (не `dprintf`) — удалить. Штраф: перезагрузка пользователя.
