...
- **Изменение файлов — только после согласования плана.** План делается в Plan Mode (`EnterPlanMode`). После одобрения плана пользователем — можно выполнять. **Никаких правок кода без одобренного плана.**
- План должен описывать: какие файлы меняются, зачем, архитектуру изменений, как проверять результат.
- **Любое изменение кода — сначала план в Plan Mode.** Даже однострочное. Без исключений.
