...
**Повторный инжект без перезапуска Minecraft запрещён.** `dlopen` с тем же путём возвращает старый хендл — обновлённый .so не загрузится. Каждый новый инжект = перезапуск игры.
