...
## BUG-004: трамплин CreateHook — rel32 обрезается при >2GB дистанции

**Симптомы**: игра крашится с `SIGSEGV SEGV_MAPERR` по адресу `XNextEvent + backupLen + N * 4GB`. Адрес краша имеет правильный offset в пределах страницы, но битый номер страницы.

**Причина**: `CreateHook` выделяет трамплин через `mmap(nullptr, ...)` — он может оказаться где угодно в адресном пространстве (подтверждено: 733 ГБ от XNextEvent). Обратный прыжок из трамплина вычисляется как `rel32 = static_cast<int32_t>(target - tramp)`. При дистанции >2GB значение не влезает в `int32_t` — обрезается. Трамплин прыгает на `XNextEvent + backupLen ± N * 4GB`.

**Подтверждение** (gdb, 2026-07-24):
```
XNextEvent:       0x7fa830cf23b0
Трамплин (mmap):  0x7fa8e8033000  (733 ГБ от XNextEvent)
Ожидалось:        jmp XNextEvent+5 = 0x7fa830cf23b5
Фактически:       jmp 0x7fa930cf23b5  (разница 0x100000000 = 4GB)
```

**Решение**: выделять трамплин рядом с target'ом через `AllocateNear()` (уже используется для bridge-страниц в `InstallJump`).
