...
**Ключевой вывод**: проблема не в X11 и не в свойствах окна Minecraft. Проблема в том, что мы вызываем ресайз **изнутри процесса Minecraft** (из инжектированной libtoolscreen.so). Когда тот же самый код вызывается из внешнего процесса — всё работает.

**Гипотезы**:
1. GLFW/LWJGL перехватывает или игнорирует ресайзные вызовы, сделанные с того же Display-соединения
2. XLockDisplay внутри процесса создаёт конфликт — рекурсивный лок или неправильная обработка событий
3. Порядок обработки событий в цикле GLFW/XNextEvent сбрасывает размер после нашего вызова

**Следующий шаг**: проверить, работает ли ресайз из инжектора, если делать его не из `DetourXNextEvent` (под локом), а из `hk_glXSwapBuffers` (рендер-поток, без лока).
